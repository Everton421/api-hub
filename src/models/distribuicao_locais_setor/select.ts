...
export class SelectDistribuicaoSetor{
    ...
    async selectByParam(empresa:string, query:Partial<IDistribuicaoLocaisSetor> ){
    ...
    if (Object.keys(query).length <= 1) {
            ...
            let sql = ` SELECT *,
             ...
             let whereClause = ' WHERE  '
               ...
               if( conditions.length > 0 ){
                    finalSql = sql + conditions.join(' AND ') + whereClause
               }
     ...
     async selectByParamUpdate(empresa:string, query:Partial<IDistribuicaoLocaisSetor> ){
        return new Promise( async (resolve, reject)=>{
    if (Object.keys(query).length <= 1) {
                        return reject(new Error("Nenhum campo fornecido para filtrar."));
                    }
